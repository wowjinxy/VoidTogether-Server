...
export default class PropModule extends ServerModule {
    ...
    SpawnProp(PropName, Position, Rotation, Scale, Velocity, PropData, OwnerId = null) {
        ...
        for (let i = 0; i < data.props.length; i++) {
            ...
            if (PropIndex == -1) {
                if (data.props[i].networkId == "*")
                    this.SpawnProp(data.props[i].name, data.props[i].position, data.props[i].rotation, data.props[i].scale, data.props[i].velocity, data.props[i].data, data.userId);
            } else {
